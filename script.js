...
function processAttendance(id) {
    ...
    if (day === 0 || day === 6) {
        return window.showToast("SISTEMA CERRADO: FIN DE SEMANA", "warning");
    }
...
function getFilteredData() {
    ...
    const calculateStatus = (sId, targetDate) => {
        const log = attendanceLog.find(l => l.studentId === sId && l.dateStr === targetDate);
        if (log) return log.status;
        
        const tDate = new Date(targetDate + 'T00:00:00');
        const tDay = tDate.getDay();
        if (tDay === 0 || tDay === 6) return 'FIN DE SEMANA';
        if (targetDate > todayStr) return 'FECHA FUTURA';
        if (targetDate < todayStr) return 'FALTA';
        return (now.getHours() >= 10 ? 'FALTA' : 'PENDIENTE');
    };
    ...
    return students.filter(s => {
        ...
        const st = calculateStatus(s.id, targetDate);
        
        const matchStatus = (fSt === "" || st === fSt);
        const matchDate = (fD === "" || attendanceLog.some(l => l.studentId === s.id && l.dateStr === targetDate) || st === 'FALTA' || st === 'FECHA FUTURA' || st === 'FIN DE SEMANA');
...
window.renderReport = () => {
    ...
    tb.innerHTML = filtered.map(s => `
        <tr class="hover:bg-zinc-50 border-b">
            ...
            <td class="px-6 py-5 font-black text-xs text-center ${
                s.st === 'FALTA' ? 'text-red-600' : 
                (s.st === 'ASISTIÓ' || s.st === 'TARDE' ? 'text-emerald-600' : 'text-plomo')
            }">${s.st}</td>
        </tr>`).join('');
};
